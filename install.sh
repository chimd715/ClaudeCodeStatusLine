#!/usr/bin/env bash
# Install Claude Code Status Line and its slash commands (/setmsg, /setmemo).
#
# Installs:
#   - statusline.sh                  → $CLAUDE_DIR/statusline.sh
#   - cleanup-statusline-msgs.py     → $CLAUDE_DIR/scripts/
#   - scripts/setmsg.sh              → $CLAUDE_DIR/scripts/setmsg.sh
#   - scripts/setmemo.sh             → $CLAUDE_DIR/scripts/setmemo.sh
#   - commands/setmsg.md             → $CLAUDE_DIR/commands/setmsg.md
#   - commands/setmemo.md            → $CLAUDE_DIR/commands/setmemo.md
#
...
install -m 755 "$SRC_DIR/scripts/setmsg.sh"           "$CLAUDE_DIR/scripts/setmsg.sh"
install -m 755 "$SRC_DIR/scripts/setmemo.sh"          "$CLAUDE_DIR/scripts/setmemo.sh"
install -m 644 "$SRC_DIR/commands/setmsg.md"          "$CLAUDE_DIR/commands/setmsg.md"
install -m 644 "$SRC_DIR/commands/setmemo.md"         "$CLAUDE_DIR/commands/setmemo.md"
...
# 3a. statusLine — only set if absent.
...
the SessionStart cleanup hook, and the /setmsg, /setmemo slash commands.
